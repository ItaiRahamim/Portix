...
import { getAccount, getInvoicesForAccount } from "../data/mockData";
        ...
        <CardContent className="pt-6">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="mt-0.5">{account.totalInvoices}</p>
            </div>
            <div>
              <p className="text-gray-500 text-xs">Total Amount</p>
              <p className="mt-0.5">${account.totalAmount.toLocaleString()}</p>
            </div>
            <div>
              <p className="text-gray-500 text-xs">Paid Amount</p>
              <p className="mt-0.5 text-green-600">${account.paidAmount.toLocaleString()}</p>
            </div>
            <div>
              <p className="text-gray-500 text-xs">Remaining Balance</p>
              <p className={`mt-0.5 ${account.remainingBalance > 0 ? "text-red-600" : "text-green-600"}`}>
                ...
                {invoices.map((inv) => (
                  <TableRow key={inv.id}>
                    ...
                    <TableCell className="text-sm text-gray-500">
                      {inv.relatedShipment || "-"} / {inv.relatedContainer || "-"}
                    </TableCell>
                    ...
                    <TableCell className="text-right text-sm">
                      {inv.remainingAmount > 0 ? (
                        <span className="text-red-600">${inv.remainingAmount.toLocaleString()}</span>
                      ) : (
                        <span className="text-green-600">$0</span>
                      )}
                    </TableCell>
                    <TableCell>
                      <span className={`text-xs px-2 py-0.5 rounded ${statusStyles[inv.status]}`}>
      ...
      <Dialog open={uploadInvoiceOpen} onOpenChange={setUploadInvoiceOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Upload a new invoice document.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { toast.success("Invoice uploaded successfully."); setUploadInvoiceOpen(false); }}>
      ...
      <Dialog open={uploadSwiftOpen} onOpenChange={setUploadSwiftOpen}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { toast.success("SWIFT document uploaded successfully."); setUploadSwiftOpen(false); }}>
              <Upload className="w-4 h-4 mr-2" />Upload
            </Button>
          </DialogFooter>
        </DialogContent>
