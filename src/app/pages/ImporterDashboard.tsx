...
export function ImporterDashboard() {
  ...
  const enrichedContainers = useMemo(() => {
    return mockContainers.map((c) => {
      ...
      else if (c.clearanceStatus === "waiting-for-review") containerStatus = "Waiting Customs Review";
  ...
  // KPIs
  const activeContainers = enrichedContainers.filter(
    (c) => c.clearanceStatus !== "released"
  ).length;
  const waitingDocs = enrichedContainers.filter(
    (c) => c.clearanceStatus === "missing-documents"
  ).length;
  const waitingReview = enrichedContainers.filter(
    (c) => c.clearanceStatus === "waiting-for-review"
  ).length;
  const rejectedContainers = enrichedContainers.filter(
    (c) => c.clearanceStatus === "rejected-action-required"
  ).length;
  const readyForClearance = enrichedContainers.filter(
    (c) =>
      c.clearanceStatus === "ready-for-clearance" ||
      c.clearanceStatus === "released"
  ).length;
              ...
              <TableBody>
                {filtered.map((c) => (
                  <TableRow
                    ...
                    onClick={() => navigate(`/importer/containers/${c.id}`)}
                  >
                    ...
                    <TableCell>
                      <span
                        className={`text-xs px-2 py-0.5 rounded whitespace-nowrap ${
                          c.containerStatus === "Released"
                            ? "bg-green-100 text-green-700"
                            : c.containerStatus === "Ready for Clearance"
                            ? "bg-emerald-100 text-emerald-700"
                            : c.containerStatus === "Rejected Documents"
                            ? "bg-red-100 text-red-700"
                            : c.containerStatus === "Documents Missing"
                            ? "bg-gray-200 text-gray-700"
                            : c.containerStatus === "In Clearance"
                    ...
                    <TableCell>
                      <span
                        className={`text-xs px-2 py-0.5 rounded whitespace-nowrap ${
                          c.customsReviewStatus === "All Approved"
                            ? "bg-green-100 text-green-700"
                            : c.customsReviewStatus === "Has Rejections"
                            ? "bg-red-100 text-red-700"
                            : c.customsReviewStatus === "Under Review"
                            ? "bg-yellow-100 text-yellow-700"
                            : "bg-gray-100 text-gray-700"
                        }`}
                      >
                        {c.customsReviewStatus}
                      </span>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div onClick={(e) => e.stopPropagation()}>
                        <Button variant="outline" size="sm" onClick={() => navigate(`/importer/containers/${c.id}`)}>
