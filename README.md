...
- [ ] **Maersk / CMA CGM API** — live carrier tracking, ETA push notifications
- [ ] **Tauri Desktop App** — offline-capable native app wrapping the Next.js frontend (Phase 2)
- [ ] **Multi-currency ledger** — EUR · USD · ILS with daily rate snapshots
- [ ] **e-BL integration** — digital Bill of Lading via SWIFT KTDDE / essDOCS
- [ ] **Predictive clearance delays** — ML model on port congestion data

---

<details>
<summary><strong>Developer Appendix — Local Setup</strong></summary>
...
```bash
git clone https://github.com/ItaiRahamim/Portix.git
cd Portix
npm install
```
...
```bash
# Apply all 25 migrations (schema, RLS, triggers, seed data)
...
├── app/
│   ├── (dashboard)/
│   │   ├── importer/          # Control tower, accounts, claims, licenses
│   │   ├── supplier/          # Document upload, cargo media, accounts
│   │   └── customs-agent/     # Compliance queue, document review
│   └── auth/                  # Supabase Auth flows
├── components/                # Shared role-aware UI components
├── lib/                       # Supabase client, type definitions, DB helpers
├── hooks/                     # TanStack Query + Realtime subscriptions
└── supabase/
    ├── migrations/            # 25 migrations — schema, RLS, triggers, seed
